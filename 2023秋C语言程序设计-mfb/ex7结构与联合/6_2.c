...
// 交换两个学生节点的数据
void Swap(STUDENTS *cur, STUDENTS *post)
{
    // 交换平均成绩值
    double tmp = cur->avg;
    cur->avg = post->avg;
    ...
    strcpy(cur->name, post->name);
    strcpy(post->name, tmp1);

    // 交换学生ID
    strcpy(tmp1, cur->uid);
    strcpy(cur->uid, post->uid);
    strcpy(post->uid, tmp1);

    // 交换学生科目成绩数组
    int tmp2[4];
    memcpy(tmp2, cur->scores, sizeof(int) * 4);
    memcpy(cur->scores, post->scores, sizeof(int) * 4);
    memcpy(post->scores, tmp2, sizeof(int) * 4);
}

// 使用冒泡排序对学生链表按照平均成绩和姓名进行排序
void bubbleSort(STUDENTS *head)
{
    ...
    do
    {
        ...
        while (ptr->next != end)
        {
            // 如果当前学生的平均成绩大于下一个学生的平均成绩，交换两个学生节点的数据
            if (ptr->avg > ptr->next->avg)
            {
                Swap(ptr, ptr->next);
                swapped = 1;
            }
            // 如果当前学生的平均成绩与下一个学生的平均成绩相等，按照姓名字典序排序
            else if (ptr->avg == ptr->next->avg)
            {
